...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['media'])) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        ...
        if (!isset($limits[$cat]) || $file['size'] > $limits[$cat]) {
            ...
            $newName      = $originalName . '_' . $username . '_' . $nextNum . '.' . $ext;
...
<head>
    ...
    <link rel="stylesheet" href="imgProj\img.css?v=20" />
</head>

<body>
    <div class="container">
    <div id="open-chat" class="btn open-chat" onclick="openChatSection()">💬</div>
    <div class="chatSection">
        <div id="close-chat" class="btn-danger" onclick="closeChatSection()"></div>
        <div class="chat-container chat" id="chat-container">
        <?php
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                echo '<div class="message">';
                echo '<span class="name">' . htmlspecialchars($row["name"]) . '</span>' . '<p class="chat-text">' . htmlspecialchars($row["message"]) . '</span><br>';
                echo '<span class="time">' . date("H:i", strtotime($row["timestamp"])) . '</span>';
                echo '</div>';
            }
        }
        ?>
        ...
        <form id="chatForm" class="chat-input">
            <input required type="text" id="name" name="name" placeholder="Your name" value="<?php echo htmlspecialchars($name); ?>" <?php if (isset($_COOKIE["username"])) echo "readonly"; ?>>
            <textarea id="message" name="message" rows="4" cols="50" maxlength="1000" placeholder="Type a message..." required></textarea>
            <button type="submit" class="btn btn-primary">Send</button>
        </form>
    </div>
        <h1>Upload Media (Image 8 MB, Video 50 MB, Audio 20 MB, Other 100 MB)</h1>
...
<div id="gallery">
    ...
    foreach ($ordered as $file):
        ...
        preg_match('/(.*)_([a-zA-Z0-9]+)_/', $file, $matches);
        $username = isset($matches[2]) ? $matches[2] : 'Unknown'; 
...
function checkAndCloseChatOnResize() {
    const currentWidth = window.innerWidth;
    if (!hasLoaded) {
        hasLoaded = true;
        if (currentWidth < 1500 && chatSectionVisible) {
            closeChatSection();
        } else if (currentWidth >= 1500 && !chatSectionVisible) {
            openChatSection();
        }
        return;
    }
...
window.addEventListener("resize", checkAndCloseChatOnResize);
window.addEventListener("load", checkAndCloseChatOnResize);
function startChatUpdate() {
    ...
    chatInterval = setInterval(() => {
        ...
        if (lastTimestamp) {
            data.append("last_timestamp", lastTimestamp);
        }

        fetch("imgProj/chat_update.php", {
            method: "POST",
            body: data,
        })
            ...
            .then((data) => {
                if (data.trim() !== "") {
                    ...
                    if (messages.length > 0) {
                        ...
                        const timestampText = lastMessage.querySelector("em").textContent;
